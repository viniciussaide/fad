...
function virou_dia($batida1,$batida2){
	...
	if ($result){
		while($row = mysqli_fetch_array($result, MYSQL_ASSOC)){
			//Variáveis que são resetadas a cada linha lida
			$pht = 0;
			$almocou1 = false;
			$jantou1 = false;
			$almocou2 = false;
			...
			//Caso mude de matrícula ou matricula vazia
			if($matricula==""){
				$flag_descanso = false;
			...
			elseif ($hora_prev_1->format('H:i')=="08:00" AND $hora_prev_4->format('H:i')=="16:00"){
				$flag_x_y = true;
			}
			elseif ($hora_prev_1->format('H:i')=="16:00" AND $hora_prev_4->format('H:i')=="00:00"){
				$flag_x_y = true;
			}
			else {
				$flag_x_y = false;
...
/* 			if (isset($hora_descanso) AND ($qt_batidas % 2==0 AND count($vetor_batidas)%2 ==1)){
				if (abs($hh_correto-$pht)>=abs($hora_descanso-$pht) AND $hora_descanso<=11 AND $hh_correto>=8 AND $hh_correto<>0){
					$horario_almoco = new Datetime($row['data']." 12:00");
					$horario_janta = new Datetime($row['data']." 20:00");
					$temp = $hh_correto;
					$hh_correto = $hora_descanso;
					$hora_descanso = $temp;
					$almocou3 = desconta_hora_alimentacao($horario_almoco,$ultima_batida,$primeira_batida,$almocou3);
					date_modify($horario_almoco, '-1 days');
					$almocou3 = desconta_hora_alimentacao($horario_almoco,$ultima_batida,$primeira_batida,$almocou3);
					$jantou3 = desconta_hora_alimentacao($horario_janta,$ultima_batida,$primeira_batida,$jantou3);
					date_modify($horario_janta, '-1 days');
					$jantou3 = desconta_hora_alimentacao($horario_janta,$ultima_batida,$primeira_batida,$jantou3);
					$flag_hh_dia_anterior = true;
					if ($almocou3){
						$hh_correto -= 1;
					}
					if ($jantou3){
			...
			//Caso seja possível compensar as horas no dia, hora extra maior que deficit de horas
